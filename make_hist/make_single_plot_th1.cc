...
void make_single_plot_th1(){
  string type[] = {"p_epi","p_mupi","p_eee_final","p_muee","p_mumumu","p_emumu","p_mumue","p_muee","fcmc","fcdt"};
  ...
  //hist list
  hist.push_back("diff_total_mass_nring3_mulike0_fp0");  
  input_type.push_back(2);mode_type.push_back(2);rebin.push_back(1);
  hist.push_back("diff_total_mom_nring3_mulike0_fp0");  
  input_type.push_back(2);mode_type.push_back(2);rebin.push_back(1);
  ...
  for(int h=0;h<hist.size();h++){
    ...
    else input = TFile::Open(Form("../output/%s.sk1_4.mode_%s_wo_livetime.root",type[input_type[h]].c_str(),type[mode_type[h]].c_str()));
