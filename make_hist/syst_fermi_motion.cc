...
void syst_fermi_motion(){
  ...
  for(int s=0;s<hist_set.size();s++){
    ...
    for(int ss=0;ss<hist_set[s].size();ss++){//decide max event of the hist
      cout << "input=" <<  Form("../output/%s.%s.mode_%s_validation.root",type[input_type_set[s].at(ss)].c_str(),period.c_str(),type[mode_type_set[s].at(ss)].c_str()) << endl;
    ...
    for(int h=0;h<hist_set[s].size();h++){
      ...
      if(h==0){
        ...
        ratio_hist->SetName("ratio_fermi_motion");
