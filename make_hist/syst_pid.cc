void syst_pid(){
  ...
  TFile *input_mc = TFile::Open("../output/fcmc.sk4.mode_subgev_onedecaye.root");//mc
  TFile *input_data = TFile::Open("../output/fcdt.sk4.mode_subgev_onedecaye.root");//mc
  TH1* hist_mc = (TH1*) input_mc->Get("prob_angle");
  TH1* hist_data = (TH1*) input_data->Get("prob_angle");
  ...
  line->SetLineWidth(2);
  line->Draw();
  c1->SaveAs("hist/compare_data_mc_prob_angle_subgev_onedecaye.pdf");
  ...
  graph->Draw("al");
  c4->SaveAs("hist/chi2_shift0_prob_angle_subgev_onedecaye.pdf");

  TCanvas *c2 = new TCanvas("c2","",800,600);
  TF1 *func = new TF1("func","[0]*x*x+[1]*x+[2]",-2,2);
  func->SetLineColor(2);
  graph2->Fit(func,"","",-2,2);
  graph2->SetMarkerStyle(8);
  graph2->Draw("ap");
  c2->SaveAs("hist/minchi2_prob_angle_subgev_onedecaye.pdf");
  ...
  line2->Draw();
  c3->SaveAs("hist/compare_data_mc_prob_angle_bestfit_subgev_onedecaye.pdf");
}
