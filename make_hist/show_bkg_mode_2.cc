void show_bkg_mode_2(){
  string type[] = {"subgev_onemulike"};
  int mode_id = 0;
  int sk_period = 4;
  int n_michel = -1;
  ...
  for(int p=0;p<2;p++){
    for (int t=0;t<2;t++){
      for (int m=0;m<54;m++){
        if(p==0) hist = (TH1*) input->Get(Form("n_michel_electron_type%d_mode_pos%d",t,m+1)); 
        if(p==1) hist = (TH1*) input->Get(Form("n_michel_electron_type%d_mode_neg%d",t,m+1)); 
        float events=0.; 
