...
HistogramService::HistogramService(const std::string &filename)
  ...
  m_dummyGraph = new TGraph("dummy2D", "dummy 2d");
  //m_dummyGraph->SetDirectory(0);
...
void HistogramService::WriteOutput() {
  ...
  for (std::vector<std::string>::const_iterator it = m_listOfTriggers.begin(); it != m_listOfTriggers.end(); ++it) {
    ...
    for (std::vector<std::string>::const_iterator jt = m_listOfSystematics.begin(); jt != m_listOfSystematics.end(); ++jt) {
      ...
      for (std::map<std::string, TGraph *>::const_iterator kt = h_histGraph[*it][*jt].begin(); kt != h_histGraph[*it][*jt].end(); ++kt) {
        h_histGraph[*it][*jt][kt->first]->Write();
      }
    }
  }
  //m_file->ls();
...
void HistogramService::createGraph(const std::string &name, const std::string &title, bool allSyst) {
  ...
  for (std::vector<std::string>::const_iterator it = m_listOfTriggers.begin(); it != m_listOfTriggers.end(); ++it) {
    m_file->cd(std::string(m_filename+std::string(":/")+*it).c_str());
    //m_file->ls();
    //h_histGraph[*it][""][name] = new TGraph(name.c_str(), title.c_str());
    h_histGraph[*it][""][name] = new TGraph();
    h_histGraph[*it][""][name]->SetName(name.c_str());
    //std::cout << "graph name is " << h_histGraph[*it][""][name]->GetName() << std::endl;
    //m_file->Add(h_histGraph[*it][""][name]);
    //h_hist2D[*it][""][name]->Sumw2();
    m_noSystHistGraph.push_back(name);
  }
}
...
TGraph *&HistogramService::graph(const std::string &name, const std::string &trigger, const std::string &systematics) {
  if ( (systematics != "") && (std::find(m_noSystHistGraph.begin(), m_noSystHistGraph.end(), name) != m_noSystHistGraph.end()) )
    return m_dummyGraph;
  // this returns a dummy histogram if the histogram was not setup
  if (h_histGraph[trigger][systematics].find(name) == h_histGraph[trigger][systematics].end())
    return m_dummyGraph;
  return h_histGraph[trigger][systematics][name];
}
