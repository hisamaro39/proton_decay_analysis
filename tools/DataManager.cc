...
DataManager::DataManager( TTree * tree )
{
...
void* DataManager::Get( const char * branch  )
{
   ...
   if( _branch_space.count( branchname ) ) 
      ...
      std::cout << " Warning in DataManager::Get - branch -" << branch << "- was not found. " << std::endl;

   return 0;
}


void DataManager::Get( const char * branch, void * ptr  )
{
   std::string branchname( branch );
...
void DataManager::SetBranch( const char * name   )
{
   TBranch * branch = 0;
   std::string ltitle;
   std::string * _str;
   ...
   if( ! branch ) { std::cerr << "DataManager::SetBranch Branch: " << branchname << " does not exist. " << std::endl; abort();} 
   ...
   if ( kVerbosity >= 2)
      std::cout << "DataManager::SetBranch Loading string: " << _str->c_str() << " " << _store->ndims << " " << std::endl;
   while( loc0 != std::string::npos && loc1 != std::string::npos )
   {
      ...
      if ( kVerbosity >= 2)
        std::cout << "DataManager::SetBranch sub: " << sub << " " << size <<  " "  << count <<  " " << _store << std::endl;
      ...
      if( size < 1 ) {
        if( ! _var_size.count(sub) )
        {
          ...
          _var_size[ sub ] = size ;
          if ( kVerbosity >= 2)
            std::cout << "DataManager::SetBranch Size of variable : " << sub << " " << size << " at finish " <<  branchname.c_str() << std::endl; 
        }
        else 
        {	
          size = _var_size[ sub ] ; 
          if ( kVerbosity >= 2)
            std::cout << "DataManager::SetBranch Using size " << size << " for var " << sub << " from var_size map " << std::endl;
        }	
      }
      
      if ( kVerbosity >= 2)
        std::cout << "DataManager::SetBranch sub: " << sub << " size: " << size << " tot: " << total_size << std::endl;
   ...
   if( total_size == 0 ) { std::cerr << "DataManager::SetBranch Size of branch : " << branchname << " computed to be zero. abort. " << std::endl; abort();} 

   _store->address = calloc( total_size , sizeof(char) );         

   if ( kVerbosity >= 2)
     std::cout << "DataManager::SetBranch Address: " << _store->address  << std::endl;
...
void DataManager::SetAllBranches( int verbose )
{
...
void DataManager::SetAllBranches( TTree * ltree, int verbose )
{
   TObjArray * branches = ltree->GetListOfBranches();

   if( branches == NULL )
   {
      std::cout << "Error in DataManager::SetAllBranches, No branches for tree " << ltree->GetName() << std::endl;
      return;
   }
   std::string name;
   ...
   for( int i = 0 ; i < nBranches ; i++ )
   {
       ...
       if( kVerbosity >= 1 ) 
         std::cout << "DataManager::SetAllBranches Setting branch >" << name  <<"<" << std::endl; 
       ...
       if( kVerbosity >= 1 ) 
            std::cout << "DataManager::SetAllBranches  .... set?    " << _branch_space.count( name )   << std::endl
...
std::vector< TString > DataManager::GetListOfKeys()
{
...
std::vector< TString > DataManager::GetListOfBranchTitles()
{
...
int DataManager::GetSize( const char * branch )
{
...
void DataManager::Get( const TString key, TypeWrapper<int>& wrapper )
{
    if( _int_data.count(key) )
    {
      ...
      std::cout << " Warning DataManager::Get " << key << " has not been registered. "
                << " There is no access to this variable " << std::endl; 
}


void DataManager::Get( const TString key, TypeWrapper<unsigned>& wrapper )
{
...
void DataManager::Get( const TString key, TypeWrapper<float>& wrapper )
{
...
void DataManager::Get( const TString key, TypeWrapper<double>& wrapper )
{
...
//
void DataManager::GetEntry( unsigned int x )
{
...
void DataManager::EnableUsedListOnly( )
{
...
void DataManager::AlignmentTest( AlignFriend * af , unsigned skip )
{

  if( !ListOfFriends )
  {
     std::cout << " DataManager::AlignmentTest  No friends specified for alignment test." << std::endl;
     return ;
  }
  std::cout << " DataManager::AlignmentTest  Now testing friend alignment. " << std::endl;
...
TArrayF * DataManager::GetTArrayF( const TString key )
{
    if( _tarrayf_data.count(key) )
    {
      ...
      std::cout << " Warning DataManager::GetTArrayF " << key << " has not been registered. "
