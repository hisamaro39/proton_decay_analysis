#ifndef _pcthruLikelihood_
#define _pcthruLikelihood_
...
class pcthruLikelihood : public LikelihoodReturn
{

  public:

    // initialize using the default likelihood variable in the cardreader 
    pcthruLikelihood(DataManager *, CardReader *, int );
 
    // to specify which variable name the likilehood file should
    // be read in from
    pcthruLikelihood(DataManager *, CardReader *, int , const char * );
    ...
    void LoadWrappers(); 
    virtual float llBuild_pcthru();
    virtual void llBuild_pcthru_read();
