...
pcstopLikelihood::pcstopLikelihood( DataManager * _dm , CardReader * reader , int skg )
:LikelihoodReturn( _dm, reader )
{
   ...
   read->GetKey("pcstopLikelihood", input );
...
pcstopLikelihood::pcstopLikelihood( DataManager * _dm , CardReader * reader , int skg, const char * varname )
:LikelihoodReturn( _dm, reader )
{
   skgen = skg;
...
void pcstopLikelihood::LoadWrappers()
{
   dm->Get("nmue", nmue);
   dm->Get("evis", evis);
   dm->Get("ip", ip);
...
float pcstopLikelihood::llBuild_pcstop( )
{
    float ll = 0.;
    ...
    if( bin > 0 )
    {
//     ll += LoadLikelihood_pcstop( 310000, float(ndcye) , bin );
//     ll += LoadLikelihood_pcstop( 320000, float(ringn) , bin );
       
       //if( dpos > 1.0e-5 )
       //   ll += LoadLikelihood( 220000, sqrt(dpos)   , bin );
       if(ringn > 1)
       {
//         ll += LoadLikelihood_pcstop( 330000, transmom, bin);
       }
...
void pcstopLikelihood::llBuild_pcstop_read()
{
...
//     ll_ndcye = LoadLikelihood_pcstop( 310000, float(ndcye) , bin );
//     ll_ringn = LoadLikelihood_pcstop( 320000, float(ringn)  , bin );

       if(ringn > 1)
       {
//         ll_transmom = LoadLikelihood_pcstop( 330000, transmom      , bin );
       } 
...
//  criteria
////////////////
int pcstopLikelihood::muedcyBuild()
{
   int i , nmuemax;
   int lmuedcy = 0;
...
int pcstopLikelihood::ringnBuild()
{
     int lringn;
...
float pcstopLikelihood::transmomBuild()
{
     int i;
...
int pcstopLikelihood::GetEnergyBin( )
{
