#ifndef _numuLikelihood_
#define _numuLikelihood_
...
class numuLikelihood : public LikelihoodReturn
{

  public:

    // initialize using the default likelihood variable in the cardreader 
    numuLikelihood(DataManager *, CardReader *, int );
 
    // to specify which variable name the likilehood file should
    // be read in from
    numuLikelihood(DataManager *, CardReader *, int , const char * );
    ...
    void LoadWrappers(); 
    virtual float llBuild_numu();
    virtual void llBuild_numu_read();
